...
function countEntries($table, $where = '', $what = '') {
	...
	else $q = "SELECT COUNT(*) FROM " . $table . " LIMIT 1";
...
function ca($page, $content) {
	if ($page == $content) echo 'active';
}

function ck($page, $content) {
	if ($page == $content) echo 'checked';
}
...
function array_key_exists_wildcard($array, $search, $return = '') {
	...
	if ($return == 'key-value') return array_intersect_key($array, array_flip($result));
	return $result;
}
...
function array_value_exists_wildcard($array, $search, $return = '') {
	...
	if ($return == 'key-value') return array_intersect($array, $result);
	return $result;
}
